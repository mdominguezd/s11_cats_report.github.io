...
Put disclaimer of how all of this report with code associated can bo
found on s11-cats-report.github.io
...
Finally, despite the efforts to mitigate the effects of caching, it's
possible that tiles from the same overview might have been reused across
different zoom levels, which could contribute to the reduced times
observed for COGs at higher zoom levels. Specifically, it was found that
the COG used in this study had maximum zoom level of 14. This means that
after that level the tiles generated use the same overview (i.e.~the one
with the highes resolution) and therefore caching of tiles might be
present.
