...
was found to be significantly lower than the one for Zarr files at
different zoom levels. These results show that COGs, being specifically
optimized for spatial data visualization, offer faster visualization
compared to Zarr files. However, this does not take away from the fact
that Zarr provides additional benefits, such as the ability to store
n-dimensional arrays. Furthermore, recent advancements like GeoZarr and
the creation of Zarr pyramids with new packages like ndpyramids could
bring significant improvements to this data format Barciauskas et al.
...
adjustments can significantly improve the speed of tiling services using
these data formats (\citeproc{ref-nasa_impact_zarr_2023}{IMPACT, 2023}).
Future considerations should include optimizing these parameters to
maximize efficiency (See
...
show a significant change for Zarrs (0.002 seconds per zoom level). The
behaviour presented here differs from the one observed by IMPACT
(\citeproc{ref-nasa_impact_zarr_2023}{2023}), where no significant
difference in rendering time was observed as a function of the zoom
level. This difference could be explained by the fact that in that
study, only the lowest zoom levels were considered, while in this study
only the highest zoom levels were taken into account, however, in order
to verify this hypothesis a broader study of the response times at more
zoom levels should be performed.
