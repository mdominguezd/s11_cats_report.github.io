...
formats are presented in two distinc subsections. The first subsection
evaluates the overall performance of the two data formats and the second
...
created from data stored as Zarr showed a significantly wider range than
...
was found to be significantly lower than the one for Zarr files at the
six first zoom levels (i.e.~the lowest resolutions).
...
\begin{verbatim}
Text(0.5, 0, 'Pyramid\n$\\rightarrow{}$zoom increase$\\rightarrow{}$')
\end{verbatim}

}

\subcaption{\label{fig-chunksize-zarr-1}Variation of chunk sizes in Zarr
file depending on zoom level.}

\centering{

\captionsetup{labelsep=none}\includegraphics{FinalReport_files/figure-pdf/fig-chunksize-zarr-output-2.pdf}

}

\subcaption{\label{fig-chunksize-zarr-2}}

}

\caption{\label{fig-chunksize-zarr}}

\end{figure}%

Different chunk sizes on Zarrs at level 0 (244x119), at level 5
(489x476), and at level 8 (978x952). The data

While chunks for COGs where constantly (512x512)

\subsubsection*{Fine tuning of dataset
creation}\label{fine-tuning-of-dataset-creation}
\addcontentsline{toc}{subsubsection}{Fine tuning of dataset creation}
