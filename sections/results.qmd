...
Additional to the simplification that eoAPI provided to deploy this using K8s, we also went for that option because it had already been tested by other authors using large datasets and heavy load requests [@munteanu_performance_2024]. @munteanu_performance_2024 perform tests with 2.3 TB of spatial data stored in S3 and a simulated simultaneous load of up to 7k users and their results showed good performance serving the data. (REVIEW)
...
data = pd.read_csv('https://github.com/mdominguezd/s11_cats_report.github.io/blob/main/sections/request_time_results_6iter.csv')
