...
The comparison of visualization speeds with TiTiler-Xarray for Zarr datasets and TiTiler-PgSTAC for COGs are presented on @fig-format-comp. In the figure it can be obeserved that on average the response for requests of COG tiles was `{python} speed_up` times faster than the one for the same file in ZARR format. Moreover, @fig-format-comp shows that the response times for tiles created from data stored as Zarr showed a significantly wider range than the ones generated from data in the COG format, which indicates more variability in the performance for Zarr.
...
As seen on @fig-comp-zoom, the zoom level of the map showed an effect on the time spent requesting and getting a tile from a tiling service for the COG format. In this study, it was found that the request times decreased by `{python} cog_slope` seconds per zoom level for COGs, and didn't show a significant change for Zarrs (`{python} zarr_slope` seconds per zoom level). The behaviour presented here differs from the one observed by @nasa_impact_zarr_2023, where no significant difference in rendering time was observed as a function of the zoom level. This difference could be explained by the fact that in that study, only the lowest zoom levels were considered, while in this study only the highest zoom levels were taken into account, however, in order to verify this hypothesis a broader study of the response times at more zoom levels should be performed.
