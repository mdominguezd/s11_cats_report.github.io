...
The results of the experiments made with different cloud-optimized data formats are presented in two distinc subsections. The first subsection evaluates the overall performance of the two data formats and the second subsection assess the performance of these data formats based on different zoom levels.
...
The results obtained are coherent to the ones previously shown by @nasa_impact_zarr_2023, where COGs' rendering time was found to be significantly lower than the one for Zarr files at the six first zoom levels (i.e. the lowest resolutions). 
...
```{python}
#| echo: false 
#| fig-cap: "Variation of chunk sizes in Zarr file depending on zoom level."
#| label: fig-chunksize-zarr

df = pd.read_csv('sections/chunksize_zarr.csv')

ax = sns.scatterplot(df, x = 'pyramids', y = 'chunksize_x', color=s11_cmap(255), label = 'Horizontal chunks (x)')
...
Different chunk sizes on Zarrs at level 0 (244x119), at level 5 (489x476), and at level 8 (978x952). The data

While chunks for COGs where constantly (512x512)

#### Fine tuning of dataset creation {.unnumbered}
